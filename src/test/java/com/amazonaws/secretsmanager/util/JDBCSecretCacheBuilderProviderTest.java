...
public class JDBCSecretCacheBuilderProviderTest {
    ...
    public void test_setRegion_configFileProperty() {
        ...
        String regionName =
                AWSSecretsManagerDriver.PROPERTY_PREFIX + "." + JDBCSecretCacheBuilderProvider.PROPERTY_REGION;
    ...
    public void test_setRegion_vpcEndpoint() {
        ...
        when(configProvider.getStringPropertyWithDefault(vpcEndpointRegion, null))
                .thenReturn("ap-southeast-3");

        SecretsManagerClient client =
                new JDBCSecretCacheBuilderProvider(configProvider).build().build();

        assertEquals(
                client.serviceClientConfiguration().endpointOverride().get().toString(), vpcEndpointUrlString);
